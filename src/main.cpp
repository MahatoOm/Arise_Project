...
//   // digitalWrite(buzzerPin, HIGH);   // try low first to keep it off
...
//     delay(2000);
//     digitalWrite(TRIGGER_PIN, LOW);
//   digitalWrite(buzzerPin, LOW);   // try HIGH first to keep it off
...
const int CAPTURE_INTERVAL_MS = 1000;   // 1 photo per second
const int PRE_ACCIDENT_SECONDS = 20;
const int POST_ACCIDENT_SECONDS = 20;

const int BUFFER_SIZE = PRE_ACCIDENT_SECONDS * 1000 / CAPTURE_INTERVAL_MS;
const int POST_COUNT = POST_ACCIDENT_SECONDS * 1000 / CAPTURE_INTERVAL_MS;

int bufferIndex = 0;
bool accidentDetected = false;
bool accidentSaved = false;

unsigned long lastCaptureTime = 0;


bool captureImage(String path) {
...
void rollingCapture() {
  ...
  if (now - lastCaptureTime >= CAPTURE_INTERVAL_MS) {
    ...
    String path = "/buffer_" + String(bufferIndex) + ".jpg";

    // This overwrites old image at the same index
    captureImage(path);
...
void saveAccidentEvent() {
  Serial.println("Saving accident event...");

  int eventNumber = millis() / 1000;

  // Save previous 20 seconds from rolling buffer
  for (int i = 0; i < BUFFER_SIZE; i++) {
    int index = (bufferIndex + i) % BUFFER_SIZE;

    String oldPath = "/buffer_" + String(index) + ".jpg";
    String newPath = "/accident_" + String(eventNumber) + "_pre_" + String(i) + ".jpg";

    File oldFile = SD_MMC.open(oldPath, FILE_READ);

    if (!oldFile) {
      Serial.println("Missing buffer file: " + oldPath);
      continue;
    }
    ...
    if (!newFile) {
      Serial.println("Failed to create: " + newPath);
      oldFile.close();
      continue;
    }
    ...
    Serial.println("Copied: " + newPath);
  }

  // Capture 10 seconds after accident
  for (int i = 0; i < POST_COUNT; i++) {
    String postPath = "/accident_" + String(eventNumber) + "_post_" + String(i) + ".jpg";
    captureImage(postPath);
    delay(CAPTURE_INTERVAL_MS);
  }

  accidentSaved = true;
  Serial.println("Accident event saved.");
}
...
void loop() {
  // Keep capturing rolling images until accident happens
  if (!accidentDetected) {
    rollingCapture();
  }
